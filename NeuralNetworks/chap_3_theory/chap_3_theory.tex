...
\paragraph{Mathematical notation}
To see how first epochs are dominated, let's do second derivatives $w'$ wrt to $w$.
$$w' = (1-\frac{\eta \lambda}{n}) w -\eta \frac{\partial C_0}{\partial w}$$
$$\frac{\partial w'}{\partial w} = (1-\frac{\eta \lambda}{n}) - \eta \frac{\partial^2 C_0}{\partial^2 w}$$
To go further, let's assume that $C_0$ is cross-entropy. Let's remember that for cross-entropy
$$\frac{\partial C}{\partial w_{jk}} = \frac{1}{n} \sum_x (a^L_j - y_j) a^{L-1}_{k}$$
so 
$$\frac{\partial^2 C}{\partial^2 w_{jk}} = \frac{1}{n} \sum_x a^{L-1}_{k} \frac{\partial a^L_j}{\partial z_{j}} \frac{\partial z^L_j}{\partial w_{jk}}$$
given that $$\sigma'(z) = \sigma(z)(1 - \sigma(z))$$,
$$\frac{\partial^2 C}{\partial^2 w_{jk}} = \frac{1}{n} \sum_x (a^{L-1}_{k})^2 a^L_j(1 - a^L_j)$$
\end{document}
