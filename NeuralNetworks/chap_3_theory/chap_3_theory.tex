...
The answer is we indeed can, but that would require changing activation function to have either no, or some form of constant, usage of previous layer. The network shall therefore adapt by changing constants. The backpropagation shall aslo update only constant(s) - essentially neurons shall be trained to react by a constant. It shall still be capabale of reacting differently to different inputs, but weights of those inputs shall stay the same forever.
